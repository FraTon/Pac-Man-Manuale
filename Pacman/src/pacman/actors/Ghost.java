package pacman.actors;

import jade.core.Agent;
import pacman.PacmanActor;
import pacman.PacmanGame;
import pacman.enums.Mode;
import pacman.enums.State;
import pacman.view.ShortestPathFinder;

import java.awt.Point;
import java.awt.Rectangle;
import java.util.ArrayList;
import java.util.List;

import behav.AssertModalita;
import behav.MossaFantasmaArancione;
import behav.MossaFantasmaAzzurro;
import behav.MossaFantasmaRosa;
import behav.MossaFantasmaRosso;
import behav.QueryOp;

/**
 * @author dzimiks
 */
public class Ghost extends PacmanActor {

	public Pacman pacman;
    public int type;
    public String color;
    public int cageUpDownCount;
    public Point[] initialPositions = {new Point(18, 11), new Point(16, 14), 
    								   new Point(18, 14), new Point(20, 14)};
    public int dx;
    public int dy;
    public static int col;
    public static int row;
    public static int direction = 0;
    public int lastDirection;
    public static int direzioneRosso;
    public static int colonnaRosso; //colonna matrice
    public static int rigaRosso; //riga matrice
    /*public static int C1; 
    public static int R1; 
    public static int D1; */

    public Mode mode = Mode.CAGE;
    public List<Integer> desiredDirections = new ArrayList<Integer>();
    public int desiredDirection;
    public static final int[] backwardDirections = {2, 3, 0, 1};
    public long vulnerableModeStartTime;
    public boolean markAsVulnerable;
    
    // Using path finder just to return the ghost to the center (cage)
    public ShortestPathFinder pathFinder; 
    
    public Ghost(PacmanGame game,Agent grafica, Pacman pacman, int type) { //31
        super(game,grafica);
        this.pacman = pacman;
        this.type = type;
        this.pathFinder = new ShortestPathFinder(game.maze);
        if(type==0)
        	color="rosso";
        if(type==1)
        	color="rosa";
        if(type==2)
        	color="azzurro";
        if(type==3)
        	color="arancione";
    }

    private void setMode(Mode mode) {//46
        this.mode = mode;
        modeChanged();
    }
    
    ///recupera tutte le immagini di ogni fantasma
    @Override
    public void init() {//45
        String[] ghostFrameNames = new String[8 + 4 + 4];
        
        for (int i = 0; i < 8; i++) 
            ghostFrameNames[i] = "/res/ghost_" + type + "_" + i + ".png";
        
        for (int i = 0; i < 4; i++) 
            ghostFrameNames[8 + i] = "/res/ghost_vulnerable_" + i + ".png";
        
        for (int i = 0; i < 4; i++) 
            ghostFrameNames[12 + i] = "/res/ghost_died_" + i + ".png";
        
        loadFrames(ghostFrameNames);
        collider = new Rectangle(0, 0, 8, 8);
        setMode(Mode.CAGE);                            //FANTASMI NELLA GABBIA
    }
    
    private int getTargetX(int col) {
        return col * 8 - 3 - 32;
    }

    private int getTargetY(int row) {
        return (row + 3) * 8 - 2;
    }
    
    //VALORE GRAFICO DELLA POSIZOINE DEL FANTASMA
    public void updatePosition() {
        x = getTargetX(col);
        y = getTargetY(row);
    }
    
    //ASSEGNA I VALOR COL E ROW ALLA POSIIONE DEL FANTASMA VERA E PROPRIA  ***********************
    private void updatePosition(int col, int row) {
        this.col = col;
        this.row = row;
        updatePosition();
    }
    
    //CALCOLA PER LO SPOSTAMENTO DEL DISEGNO DEL FANTASMA
    private boolean moveToTargetPosition(int targetX, int targetY, int velocity) {
        int sx = (int) (targetX - x);
        int sy = (int) (targetY - y);
        int vx = Math.abs(sx) < velocity ? Math.abs(sx) : velocity;
        int vy = Math.abs(sy) < velocity ? Math.abs(sy) : velocity;
        int idx = vx * (sx == 0 ? 0 : sx > 0 ? 1 : -1);
        int idy = vy * (sy == 0 ? 0 : sy > 0 ? 1 : -1);
        x += idx;
        y += idy;
        return sx != 0 || sy != 0;
    }

    private boolean moveToGridPosition(int col, int row, int velocity) {
        int targetX = getTargetX(col);
        int targetY = getTargetY(row);
        return moveToTargetPosition(targetX, targetY, velocity);
    }
    
    private void adjustHorizontalOutsideMovement() {
        if (col == 1) {
            col = 34;
            x = getTargetX(col);
        }
        else if (col == 34) {
            col = 1;
            x = getTargetX(col);
        }
    }
    
    @Override
    public void updateTitle() {
        int frameIndex = 0;
        x = pacman.x + 17 + 17 * type;
        y = 200;
        
        if (pacman.direction == 0) 
            frameIndex = 8 + (int) (System.nanoTime() * 0.00000001) % 2;
        else if (pacman.direction == 2) 
            frameIndex = 2 * pacman.direction + (int) (System.nanoTime() * 0.00000001) % 2;
        
        frame = frames[frameIndex];
    }
    
    
    //AGGIONRAMENTO POSIZIONE FANTASMA A SECONDA DELLA MODALITà DI GIOCO DEL SINGOLO FANTASMA  ****************************************************************************************
    @Override
    public void updatePlaying() {
        switch (mode) {
            case CAGE: 
            	updateGhostCage(); //possiamo non toccarlo
            	break;
            case NORMAL:
            	updateGhostNormal();  //da modificare
            	break;
            case VULNERABLE: 
            	updateGhostVulnerable();   //da modificare
            	break;
            case DIED: 
            	updateGhostDied();
            	break;
        }
        
        updateAnimation();
    }

    public void updateAnimation() {
        int frameIndex = 0;
        
        switch (mode) {
            case CAGE: 
            case NORMAL:
                frameIndex = 2 * direction + (int) (System.nanoTime() * 0.00000001) % 2;
                
                if (!markAsVulnerable) 
                    break;
                
            case VULNERABLE:
                if (System.currentTimeMillis() - vulnerableModeStartTime > 5000) 
                    frameIndex = 8 + (int) (System.nanoTime() * 0.00000002) % 4;
                else 
                    frameIndex = 8 + (int) (System.nanoTime() * 0.00000001) % 2;

                break;
            case DIED:
                frameIndex = 12 + direction;
                break;
        }
        
        frame = frames[frameIndex];
    }

    //AGGIORNAMENTO POSIZIONE FANTAMSMA CHE SI TROVA ANCORA NELLA GABBIA
    private void updateGhostCage() {
        yield:
        while (true) {
            switch (instructionPointer) {
                case 0:
                    Point initialPosition = initialPositions[type];
                    updatePosition(initialPosition.x, initialPosition.y);
                    x -= 4;
                    cageUpDownCount = 0;
                    
                    //fantasma rosso
                    if (type == 0) {
                        instructionPointer = 6;
                        break;
                    }
                    else if (type == 2) {  //fantasma azzurro
                        instructionPointer = 2;
                        break;
                    }
                    
                    instructionPointer = 1;
                case 1:
                    if (moveToTargetPosition((int) x, 134 + 4, 1)) 
                        break yield;
                    
                    instructionPointer = 2;
                case 2:
                    if (moveToTargetPosition((int) x, 134 - 4, 1)) 
                        break yield;
                    
                    cageUpDownCount++;
                    
                    if (cageUpDownCount <= type * 2) {
                        instructionPointer = 1;
                        break yield;
                    }
                    
                    instructionPointer = 3;
                case 3:
                    if (moveToTargetPosition((int) x, 134, 1)) 
                        break yield;
                    
                    instructionPointer = 4;
                case 4:
                    if (moveToTargetPosition((int) 105, 134, 1)) 
                        break yield;
                    
                    instructionPointer = 5;
                case 5:
                    if (moveToTargetPosition((int) 105, 110, 1)) 
                        break yield;
                    
                    if ((int) (2 * Math.random()) == 0) {
                        instructionPointer = 7;
                        continue yield;
                    }
                    
                    instructionPointer = 6;
                case 6:
                    if (moveToTargetPosition((int) 109, 110, 1)) 
                        break yield;
                    
                    desiredDirection = 0;
                    lastDirection = 0;
                    updatePosition(18, 11);
                    instructionPointer = 8;
                    continue yield;
                case 7:
                    if (moveToTargetPosition((int) 101, 110, 1)) 
                        break yield;
                    
                    desiredDirection = 2;
                    lastDirection = 2;
                    updatePosition(17, 11);
                    instructionPointer = 8;
                case 8:
                    setMode(Mode.NORMAL);     //FANTASMA è IN MODALITà NORMALE
                    break yield;
            }
        }
    }
    
    private PacmanCatchedAction pacmanCatchedAction = new PacmanCatchedAction();//32
    
    private class PacmanCatchedAction implements Runnable {
       
    	@Override
        public void run() {
            game.setState(State.PACMAN_DIED);
        }
    }
    
    
    
    private void updateGhostNormal() {
        if (checkVulnerableModeTime() && markAsVulnerable) {  //fantasma mangiabile?
            setMode(Mode.VULNERABLE);   
            markAsVulnerable = false;
        }
        
        //QUI NOI NON DOBBIAMO FARE DISTINZIONE DI COLORE--> SARA' ANCHE DA MODIFICARE LA FUNZIONE UPDATE GHOST MOVEMENT,***************************
        //LEVANDO  IL PRIMO ARGOMENTO CHE NON CI SERVE, E GLI ULTIMI
        
        updateGhostMovementNormal(pacman.col,pacman.row,1,pacmanCatchedAction);
        
        //fantasma rosso e rosa
        /*if (type == 0 || type == 1)      //  CREDO CHE QUI NOI DOBBIAMO INNESTAREIL NOSTRO MOVIMENTO NORMALE
            updateGhostMovement(true, pacman.col, pacman.row, 1, pacmanCatchedAction, 0, 1, 2, 3); // chase movement
        else 
            updateGhostMovement(false, 0, 0, 1, pacmanCatchedAction, 0, 1, 2, 3);*/ // random movement
    }
    
    
    
    private GhostCatchedAction ghostCatchedAction = new GhostCatchedAction();//33
    
    private class GhostCatchedAction implements Runnable {
        
    	@Override
        public void run() {
            game.ghostCatched(Ghost.this);
        }
    }
    
    
    //MOVIMENTO DEL FANTASMA CHE SCAPPA DA PAC-MAN
    private void updateGhostVulnerable() {

    	if (markAsVulnerable) 
            markAsVulnerable = false;
        
    	
    	//CHIAMA LO STESSO METODO DI PRIMA SOLO CHE GLI FA' PRENDERE LE DIREZIONI OPPOSTE    ***************** ************************************
    	// PUò ESSERE UN'OTTIMA IDEA PER IMPLEMENTARE LA FUGA DEI NOSTRI FANTASMI : SEMPLICE E FUNZIONALE--> PENSARCI SU
    	
    	//NOI DOVREMMO CREARE UN'ALTO METODO CHE CHIAMA LA FUNZIONE FUGA APPROPRIATA OPPURE 
    	// DENTRO A UPDATE GHOST MOVEMENT VALUTARE SE LA MODALITA' DEL FANTASMA è:
    	//NORMALE --> FACCIO UN TIPO DI CHIAMATA
    	//FUGA --> NE FACCIO UN'ALTRA
    	
    	//-----------------------------------------------------------------------------------------------------------------------------------
    	//ANCHE QUI MODIFICARE LA CHIAMATA LEVANDO TUTTI I PARAMENTRI CHE A NOI NON SERVONO
        updateGhostMovementNormal(pacman.col, pacman.row, 1, ghostCatchedAction); // run away movement
        //updateGhostMovementFuga(pacman.col, pacman.row, 1, ghostCatchedAction); // run away movement
        
        //metodo nuovo con questo nome che gli passo gli stessi parametri, 
        //e dentro devo rifare la stessa struttura switch colore ecc.. solo che
        //MI RIDANNO TUTTI GLI STESSI 3 PARAMETRI
        
        // return to normal mode after 8 seconds
        if (!checkVulnerableModeTime()) 
            setMode(Mode.NORMAL);
    }
    
    private boolean checkVulnerableModeTime() {
        return System.currentTimeMillis() - vulnerableModeStartTime <= 8000;
    }
    
    
    //FANTASMA MORTO MANGIATO DA PACMAN ***************************************************************
    private void updateGhostDied() {
    	
        yield:
        while (true) {
            switch (instructionPointer) {
                case 0:
                	/* SE VOGLIAMO METTERE LA NOSTRA, TOGIERE TUTTO FINO A ...*/
                    pathFinder.find(col, row, 18, 11);
                    instructionPointer = 1;
                case 1:
                    if (!pathFinder.hasNext()) {
                        instructionPointer = 3;
                        continue yield;
                    }
                    
                    Point nextPosition = pathFinder.getNext();
                    /*QUI *********************************************************
                     * CHIAMARE LA NOSTRA FUNZIONE E ASSEGNARE A COL E ROW LA POSIZIONE NUOVA
                     * 
                     * CHIAMAREMENTE AGGIONRARE I VALORI DI TUTTI I CASI*/
                    
                    col = nextPosition.x;
                    row = nextPosition.y;
                    instructionPointer = 2;
                case 2:
                    if (!moveToGridPosition(col, row, 4)) {
                        if (row == 11 && (col == 17 || col == 18)) {   //quasi arrivato?
                            instructionPointer = 3;  //lo guida in manuela invece che fargli prendere celle dal percorso 
                            continue yield;
                        }
                         
                        instructionPointer = 1;   //continua a prendere cella dal percorso -> PROLOG DEVE PASSARE LA NUOVA CELLA OPPURE MI SALVO IL PERCORSO QUI E SE LA PRENDE DA SOLO (FORSE MEGLIO)*******
                        continue yield;
                    }
                    
                    break yield;
                case 3:
                    if (!moveToTargetPosition(105, 110, 4)){
                        instructionPointer = 4;
                        continue yield;
                    }
                    break yield;
                case 4:
                    if (!moveToTargetPosition(105, 134, 4)){
                        instructionPointer = 5;
                        continue yield;
                    }
                    
                    break yield;
                case 5:
                    setMode(Mode.CAGE);  //tornatoa casa
                    instructionPointer = 4;
                    break yield;
            }
        }
    }    
    
    
    //MOVIMENTO DEL FANTASMA IN MODALITA NORMALE ( CONTROLLARE SE ANCHE IN ALTRE MODALITA')*******************************************************************************
    
   /*LASCIARE I SEGUENTI PARAMETRI  NELLA CHIAMATA :
     -targetCol
     -targetRow,
     -velocity,
     -collisionWithPacmanAction*/

    /*private void updateGhostMovement(boolean useTarget, int targetCol, int targetRow, int velocity,
			 Runnable collisionWithPacmanAction, int ... desiredDirectionsMap) {*/

    private void updateGhostMovementNormal(int targetCol, int targetRow, int velocity,
    								 Runnable collisionWithPacmanAction) {
        
  //DA QUI CANCELLEREI*************************************************************************************************************
   //**********************************************************************************

    	//Asserisco la modalità di gioco
    	agentGrafica.addBehaviour(new AssertModalita(
				agentGrafica,
				"nuovo_pacman.pl",
				"set_modalita("+color+",std)"
			));
    	
    	//VARIABILE BOOLEANA SE ASSERITA LA POSIZIONE DI PACMAN?????????????????????????????????????????????????
    	//
    	if (pacman.asserita) {
    		
    		/*agentGrafica.addBehaviour(new QueryOp(
    				agentGrafica,
    				"pacman"
    				));*/
    		
			if(color.equals("rosso")){
				
				//mossa_fantasma(colore,col,-ros,NX,NY,Direzione)
				//System.out.println(color);
				//inizializzo le variabili del fantasma rosso(riga colonna e dir) 
				agentGrafica.addBehaviour(new MossaFantasmaRosso(
						agentGrafica,
						"nuovo_pacman.pl",
						"mossa_fantasma",
						color,
						col,
						-row//,
						//"NX", //ricevo NX,NY,Direzione
						//"NY",
						//"Dir"
					));
				
			} /*else if(color.equals("azzurro")){
				//System.out.println(color);    		
				agentGrafica.addBehaviour(new MossaFantasmaAzzurro(
						agentGrafica,
						"nuovo_pacman.pl",
						"mossa_fantasma",
						color,
						col,
						-row,
						colonnaRosso,
						-rigaRosso,
						direzioneRosso,
						pacman.direction,
						"NX", //ricevo NX,NY,Direzione
						"NY",
						"Dir"
					));
				
			} else if(color.equals("rosa")) {
				//System.out.println(color);
				agentGrafica.addBehaviour(new MossaFantasmaRosa(
						agentGrafica,
						"nuovo_pacman.pl",
						"mossa_fantasma",
						color,
						col,
						-row,	    				
						pacman.direction,
						"NX", //ricevo NX,NY,Direzione
						"NY",
						"Dir"
					));
					
			}else if(color.equals("arancione")) {
				//System.out.println(color);
				agentGrafica.addBehaviour(new MossaFantasmaArancione(
						agentGrafica,
						"nuovo_pacman.pl",
						"mossa_fantasma",
						color,
						col,
						-row,
						"NX", //ricevo NX,NY,Direzione
						"NY",
						"Dir"
					));
			}*/
			/*switch(color) {
		    	case "rosso" :
		    		//mossa_fantasma(colore,col,-ros,NX,NY,Direzione)
		    		
		    		//inizializzo le variabili del fantasma rosso(riga colonna e dir) 
		    		agentGrafica.addBehaviour(new MossaFantasmaRosso(
		    				agentGrafica,
		    				"nuovo_pacman.pl",
		    				"mossa_fantasma",
		    				color,
		    				col,
		    				-row,
		    				"NX", //ricevo NX,NY,Direzione
		    				"NY",
		    				"Dir"
		    			));
		
		    	case "azzurro" :
		    		agentGrafica.addBehaviour(new MossaFantasmaAzzurro(
		    				agentGrafica,
		    				"nuovo_pacman.pl",
		    				"mossa_fantasma",
		    				color,
		    				col,
		    				-row,
		    				colonnaRosso,
		    				-rigaRosso,
		    				direzioneRosso,
		    				pacman.direction,
		    				"NX", //ricevo NX,NY,Direzione
		    				"NY",
		    				"Dir"
		    			));*/
		
		    		/*case "rosa" :
		    		agentGrafica.addBehaviour(new MossaFantasmaRosa(
		    				agentGrafica,
		    				"nuovo_pacman.pl",
		    				"mossa_fantasma",
		    				color,
		    				col,
		    				-row,	    				
		    				pacman.direction,
		    				"NX", //ricevo NX,NY,Direzione
		    				"NY",
		    				"Dir"
		    			));*/
		
		    	/*case "arancione":
		    		agentGrafica.addBehaviour(new MossaFantasmaArancione(
		    				agentGrafica,
		    				"nuovo_pacman.pl",
		    				"mossa_fantasma",
		    				color,
		    				col,
		    				-row,
		    				"NX", //ricevo NX,NY,Direzione
		    				"NY",
		    				"Dir"
		    			));
		
		    		//direction = Direzione;
		    		//COL E ROW DA INVERTIEìRE E SEGNO
			}*/
    	
    	}
    	
    	/*desiredDirections.clear();

        if (useTarget) {
            if (targetCol - col > 0) 
                desiredDirections.add(desiredDirectionsMap[0]);  //muove a destra
            else if (targetCol - col < 0) 
                desiredDirections.add(desiredDirectionsMap[2]);  //muove a sinistra

            if (targetRow - row > 0) 
                desiredDirections.add(desiredDirectionsMap[1]);  //muove sotto
            else if (targetRow - row < 0) 
                desiredDirections.add(desiredDirectionsMap[3]); //muove sopra
        }

        //scegli una direzoine a caso tra quelle individuate
        if (desiredDirections.size() > 0) {
            int selectedChaseDirection = (int) (desiredDirections.size() * Math.random());
            desiredDirection = desiredDirections.get(selectedChaseDirection);
        }
        
        yield:
        while (true) {
            switch (instructionPointer) {
                case 0:
                	
                	//TUNNEL?
                    if ((row == 14 && col == 1 && lastDirection == 2) ||
                        (row == 14 && col == 34 && lastDirection == 0)) 
                        adjustHorizontalOutsideMovement();
                    
                    double angle = Math.toRadians(desiredDirection * 90);
                    dx = (int) Math.cos(angle);
                    dy = (int) Math.sin(angle);
                    
                    //direzione confermata se: usa target, non c'è una parete e non coinde con 
                    //l'ultima direzione seguita
                    if (useTarget && game.maze[row + dy][col + dx] == 0 &&
                        desiredDirection != backwardDirections[lastDirection])
                        direction = desiredDirection;
                    else {
                    	//no target o muro o coincide con ultima direzione-> ne scelgo una a caso
                        do {
                            direction = (int) (4 * Math.random());   //direzione random
                            angle = Math.toRadians(direction * 90);
                            dx = (int) Math.cos(angle);
                            dy = (int) Math.sin(angle);
                        }
                        while (game.maze[row + dy][col + dx] == -1 ||      //muro       
                               direction == backwardDirections[lastDirection]);
                    }
                    
                    col += dx;
                    row += dy;
                    instructionPointer = 1;
                case 1:*/
                	
   // FINO A QUI E LO SOSTITUIREI CON :
                	
  /*   chiamata al metodo prolog che fa spostare il fantasma giusto e ritorna la posizione, e la direzione (MODIFICARE PROLOG)
   * 
   *  poi
   *  
   *  direction= DIREZIONE RESTITUITA DA PROLOG  MODIFICARE PROLOG
   *  
   *  col=   prima coordinata restituita da prolog
   *  row= - seconda coordinata resituita da prolog   (MI RACCOMANDO IL MENO!!!)
   *  
   * 
   *  POI LE COSE SOTTO SI LASCIANO , SI LEVA SOLO  break yield
   *  e le parentesi in eccesso (dovrebbero essere 2)
   * 
   */
                	//sposta immagine
                    if (!moveToGridPosition(col, row, velocity)) {
                        lastDirection = direction;
                        instructionPointer = 0;
//                         adjustHorizontalOutsideMovement();
                    }
                    //controlla se collide
                    if (collisionWithPacmanAction != null && checkCollisionWithPacman()) 
                        collisionWithPacmanAction.run();
                    
                    //break yield;
            //}
        //}        
    }

    //FANTASMA MANGIATO
    @Override
    public void updateGhostCatched() {
        if (mode == Mode.DIED) {
            updateGhostDied(); //modificato
            updateAnimation();
        }
    }

    
    //PACMAN MANGIATO
    @Override
    public void updatePacmanDied() {
        yield:
        while (true) {
            switch (instructionPointer) {
                case 0:
                    waitTime = System.currentTimeMillis();
                    instructionPointer = 1;
                case 1:
                    if (System.currentTimeMillis() - waitTime < 1500) 
                        break yield;
                    
                    visible = false;
                    setMode(Mode.CAGE);  //FANTASMI IN GABBIA
                    updateAnimation();
                    break yield;
            }
        }
        updateAnimation();
    }

    @Override
    public void updateLevelCleared() {
        yield:
        while (true) {
            switch (instructionPointer) {
                case 0:
                    waitTime = System.currentTimeMillis();
                    instructionPointer = 1;
                case 1:
                    if (System.currentTimeMillis() - waitTime < 1500) 
                        break yield;
                    
                    visible = false;
                    setMode(Mode.CAGE);
                    updateAnimation();
                    instructionPointer = 2;
                case 2:
                    break yield;
            }
        }
    }
   
    
    //VERIFICA SE FANTASMA ROSSO COLLIDE CON PAC-MAN
    private boolean checkCollisionWithPacman() {
        pacman.updateCollider(); //aggiora l rettangolo di collisione di pacman e del fantasma
        updateCollider();
        return pacman.collider.intersects(collider);
    }

    @Override
    public void updateCollider() {
        collider.setLocation((int) (x + 4), (int) (y + 4));
    }
    
    private void modeChanged() {//47
        instructionPointer = 0;      
    }
    
    @Override
    public void stateChanged() {//77
        if (game.getState() == State.TITLE) {
            updateTitle();
            visible = true;
        }
        else if (game.getState() == State.READY) 
            visible = false;
        else if (game.getState() == State.READY2) {
            setMode(Mode.CAGE);
            updateAnimation();
            Point initialPosition = initialPositions[type];
            updatePosition(initialPosition.x, initialPosition.y); // col, row
            x -= 4;
        }
        else if (game.getState() == State.PLAYING && mode != Mode.CAGE) 
            instructionPointer = 0;
        else if (game.getState() == State.PACMAN_DIED) 
            instructionPointer = 0;
        else if (game.getState() == State.LEVEL_CLEARED) 
            instructionPointer = 0;
    }
    
    public void showAll() {
        visible = true;
    }

    public void hideAll() {
        visible = false;
    }
    
    public void startGhostVulnerableMode() {
        vulnerableModeStartTime = System.currentTimeMillis();
        markAsVulnerable = true;
    }
    
    public void died() {
        setMode(Mode.DIED);
    }
}